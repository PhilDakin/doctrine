...
function getMainComponent(state: State, setState: Dispatch<SetStateAction<State>>) {
  switch (state) {
    case State.WELCOME: {
    ...
    <Fade in={true} timeout={1200}>
      <Paper sx={{ height: '100%', width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Paper sx={{ height: '55%', width: '40%' }} elevation={0}>
          <Typography variant='h1' color="secondary" component={"span"}>Learn faster</Typography>
          <Typography variant='h1' component={"span"}> with better summaries.</Typography>
          <Typography sx={{ marginY: 1 }} color="text.hint" variant='h6'>Doctrine provides comprehensive summaries of the information you need to know, allowing you to focus more on what matters.</Typography>
          <Button onClick={() => setState(State.ENTRY)} sx={{ marginY: 1 }} style={{ backgroundColor: theme.palette.secondary.main }} color="primary">TRY IT OUT</Button>
        </Paper>
      </Paper>
    </Fade>
  )
  ...
  return <Fade in={true} timeout={1200}>
    <Paper sx={{ height: '100%', width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <Paper sx={{ height: '55%', width: '50%' }} elevation={0}>
        <Box>
          <TextField multiline fullWidth minRows={3} maxRows={15} variant="filled" placeholder="Enter your corpus here..." />
        </Box>
        {/* TODO (pdakin): Going to need to do something with the text here... */}
        <Button onClick={() => setState(State.LOADING)} sx={{ marginY: 1 }} style={{ backgroundColor: theme.palette.secondary.main }} color="primary">SUMMARIZE</Button>
      </Paper>
    </Paper>
  </Fade>
}
...
export default function Home() {
  ...
  return (
    <Paper sx={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column' }} square={true}>
      <AppBar sx={{ height: 120, flexDirection: 'row' }} position={'relative'} elevation={0}>
        <Box sx={{ 'height': '75%', 'width': 32, 'position': 'relative', 'marginY': 1.9 }}>
